...
const isResizing = computed(() => globalStore.currentUserIsResizingSidebar)
const start = (event) => {
  if (utils.isMultiTouch(event)) { return }
  globalStore.currentUserIsResizingSidebar = true
}

const remove = (event) => {
  globalStore.currentUserIsResizingSidebar = false
  userStore.updateUser({ sidebarResizeWidth: consts.defaultDialogWidth })
}
</script>

<template lang="pug">
.right-resize.sidebar-resize-wrap.bottom-button-wrap(@click.stop)
  .inline-button-wrap(
