...
dialog.card-details(v-if="visible" :open="visible" ref="dialogElement" @click.left="closeDialogs" @keyup.stop.backspace="removeCard" :style="styles" :data-card-id="card.id")
  ...
  section
    ...
    .rows
      ...
      ItemActions(:visible="shouldShowItemActions && canEditCard" :cards="[card]" @closeDialogs="closeDialogs" :class="{ 'last-row': !rowIsBelowItemActions }" :tagsInCard="tagsInCard" :backgroundColorIsFromTheme="true")
      ...
      ItemDetailsDebug(:item="card" :keys="['listId', 'listPositionIndex', 'resizeWidth', 'x', 'y']")
    CardDetailsResize
</template>
