...
dialog.import-arena-channel.narrow(v-if="visible" :open="visible" @click.left.stop ref="dialog")
  ...
  template(v-if="accessToken")
    ...
    section
      button(@click.left="forgetaccessToken") Forget Me
...
export default {
  ...
  computed: {
    ...
    accessToken () { return this.$store.state.currentUser.accessToken },
    isAuthenticatingWithArena () { return this.$store.state.isAuthenticatingWithArena }
  },
  methods: {
    forgetaccessToken () {
      this.$store.dispatch('currentUser/accessToken', '')
