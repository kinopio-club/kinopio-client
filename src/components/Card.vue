...
const otherCardUrle = computed(() => utils.urlFromSpaceAndCard({ cardId: props.card.linkToCardId, spaceId: props.card.linkToSpaceId }))
...
const resizeWidth = computed(() => {
  ...
  if (embedIsVisible.value || isLoadingUrlPreview.value) {
    resizeWidth = Math.max(resizeWidth, consts.minCardEmbedWidth)
  }
  if (!resizeWidth) { return }
  return resizeWidth
})
const tiltDegrese = computed(() => props.card.tilt)
...
const frameId = computed(() => props.card.frameId)
...
const shouldNotStick = computed(() => {
  ...
  if (embedIsVisible.value) { return true }
...
const isCardFilteredByFrame = computed(() => {
  ...
  const hasFrame = frameIds.includes(frameId.value)
  return hasFrame
})
...
//   if (embedIsVisible.value) { isVisible = true }
...
const updateStickyStretchResistance = () => {
  ...
  const size = {
    s: isSize(width, height, 250),
    m: isSize(width, height, 500),
    l: isSize(width, height, 1000),
...
const checkIfShouldUpdateCardConnectionPaths = async (width) => {
  await nextTick()
  await nextTick()
  if (state.prevNameLineMinWidth !== width) {
    store.dispatch('currentConnections/updatePaths', { cardId: props.card.id, shouldUpdateApi: true })
  }
  state.prevNameLineMinWidth = width
}
...
const updateCardConnectionPathsIfOpenSpace = () => {
...
const updateCardConnectionPaths = async () => {
  await nextTick()
  await nextTick()
  store.dispatch('currentConnections/updatePaths', { cardId: props.card.id, shouldUpdateApi: true })
}
