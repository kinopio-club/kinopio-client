  ...
  .card(
    ...
    .bottom-button-wrap
      ...
      .resize-button-wrap.inline-button-wrap(
        ...
        button.inline-button.resize-button(tabindex="-1" :style="{background: selectedColor || 'transparent'}")
    ...
    span.card-content-wrap(:style="{width: resizeWidth, 'max-width': resizeWidth }")
        ...
        .name-wrap
          ...
          .checkbox-wrap(v-if="hasCheckbox" @mouseup.left="toggleCardChecked" @touchend.prevent="toggleCardChecked")
            label(:class="{active: isChecked, disabled: !canEditSpace}")
              input(type="checkbox" v-model="checkboxState")
          //- Name
          p.name.name-segments(v-if="normalizedName" :style="{background: selectedColor || remoteSelectedColor || 'transparent', minWidth: nameLineMinWidth + 'px'}" :class="{'is-checked': isChecked, 'has-checkbox': hasCheckbox, 'badge badge-status': Boolean(formats.image || formats.video)}")
            template(v-for="segment in nameSegments")
              NameSegment(:segment="segment" @showTagDetailsIsVisible="showTagDetailsIsVisible" @showLinkDetailsIsVisible="showLinkDetailsIsVisible")
      ...
      span.card-buttons-wrap(:class="{'tappable-area': nameIsOnlyMarkdownLink}")
        ...
        .lock-button-wrap.inline-button-wrap(v-if="isLocked" @mouseup.left="unlockCard" @touchend="unlockCard")
          button.inline-button(tabindex="-1" :style="{background: selectedColor || 'transparent'}")
        ...
        template(v-else)
          ...
          a.url-wrap(v-if="cardButtonUrl && !isComment" :href="cardButtonUrl" @click.left.stop="openUrl($event, cardButtonUrl)" @touchend.prevent="openUrl($event, cardButtonUrl)" :class="{'connector-is-visible': connectorIsVisible}")
            .url.inline-button-wrap
              button.inline-button(:style="{background: selectedColor || 'transparent'}" tabindex="-1")
          ...
          .connector.inline-button-wrap(
            ...
            button.inline-button(:class="{ active: isConnectingTo || isConnectingFrom}" :style="{background: selectedColor || 'transparent' }" tabindex="-1")
              .connected-colors
